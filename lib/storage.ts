import { StorageBucket, FileListResponse, ProjectDocument } from '@/types/storage';
...
export class StorageService {
   ...
   */
  static async listFiles(bucket: StorageBucket, path?: string): Promise<FileListResponse[]> {
    try {
      console.log(`Listing files in bucket: ${bucket}`);
      
      // Add timeout to prevent hanging
      const timeoutPromise = new Promise<never>((_, reject) => 
        setTimeout(() => reject(new Error(`Timeout listing files in bucket: ${bucket}`)), 10000)
      );
   ...
   */
  static async getDownloadUrl(bucket: StorageBucket, path: string, expiresIn = 3600): Promise<string> {
    try {
   ...
   */
  static async downloadFile(bucket: StorageBucket, path: string): Promise<Blob> {
    try {
      const { data, error } = await supabase.storage
        .from(bucket)
        .download(path);
   ...
   */
  static async deleteFile(bucket: StorageBucket, paths: string[]): Promise<void> {
    try {
   ...
   * Get file info
   */
  static async getFileInfo(bucket: StorageBucket, path: string): Promise<FileListResponse | null> {
    try {
  ...
  static async getProjectFiles(projectId?: string): Promise<{
    ...
    try {
      ...
      const preconstructionDocs = results[0].status === 'fulfilled' ? results[0].value : [];
      const constructionDocs = results[1].status === 'fulfilled' ? results[1].value : [];
      const esgReports = results[2].status === 'fulfilled' ? results[2].value : [];
      ...
      const totalFiles = preconstructionDocs.length + constructionDocs.length + esgReports.length;
      if (totalFiles === 0 && user) {
        console.warn('🚨 RLS ISSUE DETECTED: User is authenticated but no files found in any bucket');
        console.warn('   This usually means Row Level Security policies are not configured properly');
        console.warn('   👉 Solution: Apply RLS policies in Supabase SQL Editor');
      ...
      const filterByProject = (files: FileListResponse[], projectId?: string) => {
   ...
   */
  static async validateBuckets(): Promise<{ exists: string[]; missing: string[] }> {
    try {
      ...
      const requiredBuckets: StorageBucket[] = ['preconstruction-docs', 'construction-docs', 'esg-reports'];
      const existingBucketNames = buckets?.map(b => b.name) || [];
      
      const exists = requiredBuckets.filter(bucket => existingBucketNames.includes(bucket));
      const missing = requiredBuckets.filter(bucket => !existingBucketNames.includes(bucket));

      console.log('Bucket validation:', { exists, missing, allBuckets: existingBucketNames });
